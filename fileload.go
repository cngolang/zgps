...
func MultithreadDownloader(cnum int, pdst string, durl string) {
	...
	if Down_Proxy != "" { //	下载音频代理
		dialer, err := proxy.SOCKS5("tcp", Down_Proxy, nil, proxy.Direct)
		if err != nil {
	...
	request.Header.Add("Cookie", "status_convertTime=00%3A00; statuscurrentTime=0.6635")
	request.Header.Add("User-Agent", GetRandomUserAgent())
	request.Header.Add("Referer", durl)
